...
export function SongQueue({ queue, onVote }: SongQueueProps) {
  ...
  // Auto-sort queue by votes in descending order
  useEffect(() => {
    const sorted = [...queue].sort((a, b) => b.votes - a.votes);
    setSortedQueue(sorted);
  }, [queue]);

  // Top 5 songs for chart display
  const displayQueue = sortedQueue.slice(0, 5);
  // Remaining songs
  const remainingSongs = sortedQueue.slice(5);
  const maxVotes = Math.max(...displayQueue.map(s => s.votes), 1);
