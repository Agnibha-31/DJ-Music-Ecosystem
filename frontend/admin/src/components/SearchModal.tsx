...
export function SearchModal({ isOpen, onClose, onAddSong }: SearchModalProps) {
  const [searchTerm, setSearchTerm] = useState('');

  // Flatten all songs from all genres
  const allSongs = Object.entries(SONG_DATABASE).flatMap(([genre, songs]) =>
  ...
  const handleSelectSong = (song: { title: string; artist: string; genre: string }) => {
    onAddSong(song);
    setSearchTerm('');
    onClose();
  };
