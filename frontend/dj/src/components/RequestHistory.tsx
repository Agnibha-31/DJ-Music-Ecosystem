...
const RequestHistory = memo(function RequestHistory({ acceptedSongs, rejectedSongs, onRevertAccepted, onRevertRejected }: RequestHistoryProps) {
  // Memoize history computation
  const allHistory = useMemo(() => 
    [
      ...acceptedSongs.map(s => ({ ...s, status: 'accepted' as const })),
      ...rejectedSongs.map(s => ({ ...s, status: 'rejected' as const })),
    ].sort((a, b) => b.timestamp - a.timestamp).slice(0, 8),
    [acceptedSongs, rejectedSongs]
  );
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-2 max-h-[280px] overflow-y-auto custom-scrollbar">
            {allHistory.map((item, index) => (
              ...
              >
                ...
                {item.status === 'accepted' && (
                  <motion.button
                    onClick={() => onRevertAccepted(item.id, item.title, item.artist)}
                    className="p-1 rounded-md bg-red-500/20 hover:bg-red-500/30 border border-red-500/40 opacity-0 group-hover:opacity-100 transition-opacity flex-shrink-0"
                ...
                {item.status === 'rejected' && (
                  <motion.button
                    onClick={() => onRevertRejected(item.id, item.title, item.artist)}
                    className="p-1 rounded-md bg-green-500/20 hover:bg-green-500/30 border border-green-500/40 opacity-0 group-hover:opacity-100 transition-opacity flex-shrink-0"
                    whileHover={{ scale: 1.1 }}
                    whileTap={{ scale: 0.9 }}
