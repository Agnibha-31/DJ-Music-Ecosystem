...
const STORAGE_REQUIREMENTS: StorageConfig[] = [
  {
 ...
 */
const ensureFile = async (config: StorageConfig): Promise<void> => {
...
export const bootstrapStorage = async (): Promise<void> => {
  ...
  try {
    ...
    await Promise.all(
      STORAGE_REQUIREMENTS.map(async (config) => {
 ...
 */
export const validateStorage = async (): Promise<boolean> => {
  try {
    await Promise.all(
      STORAGE_REQUIREMENTS.map(async (config) => {
        const fullPath = path.resolve(process.cwd(), config.filePath);
