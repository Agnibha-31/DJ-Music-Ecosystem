...
export type SongCatalog = Record<string, Array<{ title: string; artist: string }>>;
...
const loadCatalogFromDisk = async (): Promise<SongCatalog> => {
  const fileContent = await readFile(catalogPath, "utf-8");
  return JSON.parse(fileContent) as SongCatalog;
};

export const getSongCatalog = async (): Promise<SongCatalog> => {
  if (catalogCache) {
    return catalogCache;
  }
...
const refreshCatalogCache = async (reason: string) => {
  try {
    catalogCache = await loadCatalogFromDisk();
