...
const loadQueue = async () => readJson<{ schema_version: string; records: any[] }>(queuePath);
...
const ensureSongInVenue = async (songId: string | null | undefined, venueId: string) => {
...
router.post("/queue/request", authenticateOptional, authorize("queue.request"), async (req, res) => {
  ...
  const { songId, songTitle, artist, genre, venue_id, live_session_id } = req.body as {
    songId?: string | null;
    songTitle: string;
    artist: string;
    genre: string;
...
router.patch("/queue/vote", authenticateOptional, authorize("queue.vote"), async (req, res) => {
  const actor = getActor(req);
  const { queueItemId, venue_id } = req.body as { queueItemId: string; venue_id?: string };
  const venueId = requireVenueId(venue_id, res);
  if (!venueId) return;

  const { before, updated } = await updateJsonAtomically<
    { schema_version: string; records: any[] },
    { before: any | null; updated: any | null }
  >(queuePath, async (data) => {
...
router.patch("/queue/:id/status", requireAuth, authorize("queue.admin"), async (req, res) => {
  const actor = getActor(req);
  const { status, venue_id } = req.body as { status: string; venue_id?: string };
  const venueId = requireVenueId(venue_id, res);
  if (!venueId) return;
  const data = await loadQueue();
  const index = data.records.findIndex((item) => item.id === req.params.id && !item.deleted_at && item.venue_id === venueId);
...
router.patch("/queue/:id/priority", requireAuth, authorize("queue.admin"), async (req, res) => {
  const actor = getActor(req);
  const { priority, venue_id } = req.body as { priority: number; venue_id?: string };
  const venueId = requireVenueId(venue_id, res);
  if (!venueId) return;
  const data = await loadQueue();
  const index = data.records.findIndex((item) => item.id === req.params.id && !item.deleted_at && item.venue_id === venueId);
