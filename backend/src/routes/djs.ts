...
router.get("/djs", requireAuth, authorize("djs.admin"), async (_req, res) => {
  const data = await readJson<{ records: any[] }>(djsPath);
...
router.get("/djs/me/venue", requireAuth, authorize("dj.basic"), async (req, res) => {
  ...
  const djsData = await readJson<{ records: any[] }>(djsPath);
  ...
  const venuesData = await readJson<{ records: any[] }>(venuesPath);
  ...
  const sessionsData = await readJson<{ records: any[] }>(sessionsPath);
  const activeSession = sessionsData.records.find(
    (s) => s.djId === djId && s.venueId === venueId && s.status === "active" && !s.deleted_at
  ) ?? null;
...
router.post("/dj-access-request", async (req, res) => {
  try {
    ...
    const sessionsData = await readJson<{ records: any[] }>(sessionsPath);
    const activeSession = sessionsData.records.find(
      (s) => s.djId === dj.id && s.status === "active" && !s.deleted_at
    );
    if (activeSession) {
    ...
    const requestsData = await readJson<{ records: any[] }>(accessRequestsPath);
    const existingPending = requestsData.records.find(
      (r) => r.djId === dj.id && r.status === "pending" && !r.deleted_at
    );
    ...
    const actor: Actor = { actor_type: "dj", actor_id: dj.id };
    const request = {
...
router.get("/dj-access-requests", requireAuth, authorize("djs.admin"), async (_req, res) => {
  const data = await readJson<{ records: any[] }>(accessRequestsPath);
...
router.patch("/dj-access-requests/:id/approve", requireAuth, authorize("djs.admin"), async (req, res) => {
  try {
    ...
    const sessionsData = await readJson<{ records: any[] }>(sessionsPath);
    const activeSession = sessionsData.records.find((s: any) =>
      s.venueId === venueId && s.status === "active" && !s.deleted_at
    );

    const djsData = await readJson<{ records: any[] }>(djsPath);

    if (activeSession && activeSession.djId !== request.djId) {
      const activeDj = djsData.records.find((d: any) => d.id === activeSession.djId);
    ...
    const suspendedIndex = sessionsData.records.findIndex(
      (s: any) => s.djId === request.djId && s.venueId === venueId && s.status === "suspended" && !s.deleted_at
    );

    let sessionToUse: any;
    if (suspendedIndex !== -1) {
