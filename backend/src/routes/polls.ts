...
const loadPolls = async () => readJson<{ schema_version: string; records: any[] }>(pollsPath);
const loadPollSongs = async () => readJson<{ schema_version: string; records: any[] }>(pollSongsPath);
const loadPollVotes = async () => readJson<{ schema_version: string; records: any[] }>(pollVotesPath);
const loadSongs = async () => readJson<{ schema_version: string; records: any[] }>(songsPath);

const requireVenueId = (value: any, res: any) => {
...
const ensureSongsInVenue = async (songIds: string[], venueId: string) => {
...
const buildPollPayload = (poll: any, pollSongs: any[], pollVotes: any[]) => {
...
router.get("/polls", requireAuth, authorize("polls.admin"), async (req, res) => {
  ...
  if (liveSessionId) items = items.filter((item) => item.live_session_id === liveSessionId);
...
router.post("/polls", requireAuth, authorize("polls.admin"), async (req, res) => {
  ...
  const { title, songs, venue_id, live_session_id } = req.body as { title: string; songs: string[]; venue_id?: string; live_session_id?: string };
...
router.patch("/polls/:id/vote", requireAuth, authorize("polls.admin"), async (req, res) => {
  const actor = getActor(req);
  const { songId, voterId, venue_id } = req.body as { songId: string; voterId?: string; venue_id?: string };
  const venueId = requireVenueId(venue_id, res);
  if (!venueId) return;
  const pollsData = await loadPolls();
  const pollVotesData = await loadPollVotes();
