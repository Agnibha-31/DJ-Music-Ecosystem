...
const issueTokens = (payload) => {
    const accessToken = jwt.sign(payload, env.jwtSecret, { expiresIn: env.jwtAccessTtl });
    const refreshToken = jwt.sign(payload, env.jwtRefreshSecret, { expiresIn: env.jwtRefreshTtl });
...
router.post("/auth/admin/login", async (req, res) => {
    ...
    const data = await readJson(authPath("admin_accounts.json"));
    const account = data.records.find((item) => item.status === "active" &&
    ...
    const permsData = await readJson(authPath("permissions.json"));
    ...
    const refreshData = await readJson(authPath("refresh_tokens.json"));
...
router.post("/auth/admin/signup", async (req, res) => {
    ...
    const data = await readJson(authPath("admin_accounts.json"));
    const exists = data.records.find((item) => item.username === normalizedUsername || item.email === normalizedEmail);
    if (exists) {
...
router.post("/auth/dj/login", async (req, res) => {
    ...
    const djData = await readJson(collectionPath("djs.json"));
    ...
    const sessionsData = await readJson(collectionPath("live_sessions.json"));
    let activeSession = sessionsData.records.find((s) => s.djId === account.id && s.venueId === account.venueId && s.status === "active" && !s.deleted_at) ?? null;
    // If no active session, check for a suspended one and reactivate it
    if (!activeSession) {
        const suspendedIndex = sessionsData.records.findIndex((s) => s.djId === account.id && s.venueId === account.venueId && s.status === "suspended" && !s.deleted_at);
        if (suspendedIndex !== -1) {
...
router.post("/auth/guest", async (_req, res) => {
    ...
    const anonData = await readJson(authPath("anonymous_sessions.json"));
...
router.post("/auth/logout", async (req, res) => {
    const { jti } = req.body;
    const denyData = await readJson(authPath("jti_denylist.json"));
