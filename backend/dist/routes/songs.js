...
const loadSelections = async () => readJson(selectionsPath, { records: [] });
const songKey = (title, artist, genre) => `${title.trim().toLowerCase()}::${artist.trim().toLowerCase()}::${genre.trim().toLowerCase()}`;
const resolveSelectedSongs = async (selectedGenres, selectedSongs) => {
...
const filterByVenue = (records, venueId) => {
    if (!venueId)
        return records;
...
router.post("/songs/selection", requireAuth, authorize("songs.admin"), async (req, res) => {
    ...
    const selectedGenres = Array.isArray(req.body.selectedGenres)
        ? req.body.selectedGenres.map((genre) => String(genre)).filter(Boolean)
        : [];
    ...
    const selectedSongs = selectedSongsInput.map((song) => ({
    ...
    const resolvedSongs = await resolveSelectedSongs([], combinedSongsInput);
    ...
    const updatedVenueRecords = [...venueRecords];
...
router.get("/songs", requireAuth, authorize("songs.admin"), async (req, res) => {
    ...
    const filtered = filterByVenue(data.records.filter((item) => !item.deleted_at), venueId);
...
router.get("/songs/catalog", authenticateOptional, authorize("songs.catalog.read"), async (req, res) => {
    ...
    const data = await loadSongs();
    const filtered = filterByVenue(data.records.filter((item) => !item.deleted_at && item.status === "enabled"), venueId);
    res.json({ items: filtered });
});
...
router.get("/songs/genres", authenticateOptional, authorize("songs.catalog.read"), async (req, res) => {
    ...
    const data = await loadSongs();
    const genres = Array.from(new Set(filterByVenue(data.records.filter((item) => !item.deleted_at), venueId).map((item) => item.genre)));
    res.json({ genres });
});
...
router.get("/songs/by-genre", authenticateOptional, authorize("songs.catalog.read"), async (req, res) => {
    ...
    const data = await loadSongs();
    const filtered = filterByVenue(data.records.filter((item) => !item.deleted_at && item.genre === genre), venueId);
    res.json({ items: filtered });
});
...
router.get("/songs/search", authenticateOptional, authorize("songs.catalog.read"), async (req, res) => {
    ...
    const data = await loadSongs();
    const items = filterByVenue(data.records.filter((item) => !item.deleted_at && `${item.title} ${item.artist}`.toLowerCase().includes(q)), venueId);
    res.json({ items });
});
...
router.patch("/songs/bulk-status", requireAuth, authorize("songs.admin"), async (req, res) => {
    const actor = getActor(req);
    const { ids, status, venue_id } = req.body;
    if (!venue_id) {
        return res.status(400).json({ error: "venue_id is required" });
    }
    const data = await loadSongs();
