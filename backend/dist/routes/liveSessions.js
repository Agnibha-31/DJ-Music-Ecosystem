...
const loadSessions = async () => readJson(sessionsPath);
...
router.get("/live-sessions", requireAuth, authorize("djs.admin"), async (req, res) => {
    ...
    if (venueId)
        items = items.filter((s) => s.venueId === venueId);
    if (djId)
        items = items.filter((s) => s.djId === djId);
    if (status)
...
router.get("/live-sessions/active", authenticateOptional, async (req, res) => {
    ...
    const data = await loadSessions();
    const active = data.records.find((s) => s.venueId === venueId && s.status === "active" && !s.deleted_at) ?? null;
